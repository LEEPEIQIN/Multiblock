...
for epoch in range(4):
    running_loss=0.0
    for i in range(500):
        HR,LR=common.generator("2_HR","2_LR_afternet2",16,48)
        HR=HR.to(device)
        LR=LR.to(device)
        optimizer.zero_grad()
        outputs=net2(LR)
    ...
    torch.save(net2.state_dict(), 'real_net3.pt')
print('Finished Training net')



del net2
net2_plus=common.inherit('real_net3.pt')
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
net2_plus.to(device)


criterion = nn.L1Loss()
optimizer=torch.optim.Adam(net2_plus.parameters(), lr=0.0001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
for epoch in range(6):
    ...
    print(PSNR)
    del PSNR,n_test,temp_HR_2,temp_LR_2
    torch.save(net2_plus.state_dict(), 'real_net3_plus.pt')
