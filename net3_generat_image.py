...
class Net3_plus(nn.Module):
    def __init__(self):
        super(Net3_plus, self).__init__()
...
net3_plus=Net3_plus()
net3_plus.load_state_dict(torch.load('real_net3_plus.pt'))
net3_plus.eval()
...
os.mkdir('2_LR_afternet3')
os.chdir('2_LR_afternet3')
os.mkdir('2')
os.chdir('2')
...
with torch.no_grad():
    for i in range(len(LR_set)):
        ...
        LR_1=net3_plus(LR_tl).data
        LR_2=net3_plus(LR_tr).data
        LR_3=net3_plus(LR_bl).data
        LR_4=net3_plus(LR_br).data
        ...
        os.chdir(wd_1)
        if i<9:
            LR_after.save("2_LR_afternet3_00"+str(i+1)+".png","PNG")
        else:
            LR_after.save("2_LR_afternet3_0"+str(i+1)+".png","PNG")
        os.chdir(cwd)
        print('done')
        
##
        
LR_URL='2_LR_test_afternet2'

LR_set=torchvision.datasets.ImageFolder(root=LR_URL,transform=transforms.ToTensor())
LR_loader=torch.utils.data.DataLoader(LR_set,batch_size=1,shuffle=False,num_workers=0)
LR_loader=iter(LR_loader)

os.mkdir('2_LR_test_afternet3')
os.chdir('2_LR_test_afternet3')
os.mkdir('2')
os.chdir('2')
...
with torch.no_grad():
   for i in range(len(LR_set)):
       ...
       os.chdir(wd_2)
       if i<9:
            LR_after.save("2_LR_test_afternet3_00"+str(i+1)+".png","PNG")
       else:
            LR_after.save("2_LR_test_afternet3_0"+str(i+1)+".png","PNG")
       os.chdir(cwd)
       print('done')